export default function ProgressBar({
  ...
  return (
    <div className="rounded-lg border border-[#dbcdbd] bg-white p-3">
      ...
      <div className="h-2 overflow-hidden rounded-full bg-[#e6dacd]">
        <div
          className="h-full rounded-full bg-[#1f4d45] transition-all duration-500"
          style={{ width: `${percent}%` }}
        />
      </div>
      <p className="mt-1.5 text-[11px] text-[#7f7265]">{etaLabel ? `ETA: ${etaLabel}` : "Estimating..."}</p>
    </div>
  );
}
